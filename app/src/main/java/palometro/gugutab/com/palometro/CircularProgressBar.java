...
/**
    ...
    Modified from https://github.com/passsy/android-HoloCircularProgressBar
    implemented cfraz89 pull request (Circular Thumb) and option to add makers (divisors) added
...
public class CircularProgressBar extends View {
     ...
     */
    private int mCircleStrokeWidth = 13;
        ...
        if (attributes != null) {
            try {
                setProgressColor(attributes
                        .getColor(R.styleable.CircularProgressBar_progress_color, Color.CYAN));
                ...
                setWheelSize((int) attributes
                        .getDimension(R.styleable.CircularProgressBar_stroke_width, 10));
                setThumbEnabled(attributes
                        .getBoolean(R.styleable.CircularProgressBar_thumb_visible, true));
                setMarkerEnabled(attributes
                        .getBoolean(R.styleable.CircularProgressBar_marker_visible, true));
                setThumbShape(ThumbShape.values()[
                        attributes.getInt(R.styleable.CircularProgressBar_thumb_shape, 0)
                        ]);

                mGravity = attributes
                        .getInt(R.styleable.CircularProgressBar_android_gravity,
                                Gravity.CENTER);
            } finally {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        // draw markers (divisors)
        if (markerNumber>0) {

            for (int i = 0; i < markerNumber; i++) {
                canvas.save();
                float markerRotation = (((float)360 / markerNumber) * i) - 90;
                canvas.rotate(markerRotation);
                if(progressRotation - (float)90 > markerRotation){
                    canvas.drawLine((float) (mThumbPosX + mThumbRadius / 2 * 1.4), mThumbPosY, (float) (mThumbPosX - mThumbRadius / 2 * 1.4), mThumbPosY, mProgressColorPaint);
                } else{
